...
def modify_openapi_yaml(file_path):
    """ Load YAML, apply transformations, and save changes as a new file. """
    ...
    # Create a new filename with "patched-" prefix
    directory, filename = os.path.split(file_path)
    patched_filename = os.path.join(directory, f"patched-{filename}")

    # Write the modified YAML to the new file
    with open(patched_filename, "w") as file:
        yaml.dump(data, file, default_flow_style=False, sort_keys=False)

    print(f"✅ YAML modifications applied successfully.")
    print(f"🔍 Compare the original and patched file: {file_path} ⇄ {patched_filename}")
